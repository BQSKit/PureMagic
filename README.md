...
So here's an added detail: each operator must have a preallocated set of qubits that it will run on. So when we generate a random operator for a circuit, each X, Z or Y in there must be accompanied by a randomly selected qubit (and the Y must have two randomly selected qubits). Note that in a given timestep, each operator must be assigned to a different set of random qubits, i.e. they can't overlap. Then the scheduling problem is actually just finding a route from the allocated qubits with their labels (X, Y) to a magic state for each operator, i.e. we are not finding the data qubits, instead we already have requirements for them. So the idea would be to start at the data qubits and search from there up to find a single magic state. This is where the steiner tree comes in - it connects a magic state with the set of required data qubits. So one idea is to find the best Steiner tree for each operator in turn, choosing between one or more trees anchored at one or more magic nodes. And as we iterate through the operators, we are removing all nodes associated with the prior selection.