...
Each circuit above has a matching `<name>.cliffordt.qasm` (the `compile_cliffordt` output,
checked in for reproducibility). `benchmarks` lists the eleven circuit stems; `compile-all.sh`
and `transpile-all.sh` drive `compile_cliffordt`/`transpile` over that list (the latter sweeps
`transpile`'s `-m`/`--auto` weight options for scheduler-side ablation studies). `.trans*` and
`out*` files these scripts produce are gitignored scratch output, not part of the repo.

Two subdirectories hold size-scaling families used for ablation studies rather than the main
eleven-circuit comparison, each with its own `compile-all.sh`:

- `heisenberg/` -- the Heisenberg model at `N4` through `N225` (the same
  [Benchpress](https://github.com/Qiskit/benchpress) family `square_heisenberg_N225.qasm` above
  is one instance of).
- `qft/` -- QFT at `N010` through `N220`, generated by this repo's own
  [`data/circuit-generators/gen_qft.py`](../circuit-generators/gen_qft.py) (hence the `_qiskit` suffix), independent of the
  QASMBench-sourced `qft_n160.qasm` above.
...
repo's own [`data/circuit-generators/gen_grover.py`](../circuit-generators/gen_grover.py) (`--qubits 100 --iterations 10`; iteration count
